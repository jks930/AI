      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 mt-12">
          {features.map((feature, index) => (
            <div
              key={index}
              className={`rounded-[32px] p-8 lg:p-10 transition-all duration-300 group flex flex-col h-full border ${feature.bg === 'bg-white'
            ...
            >
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-8 ${feature.bg === 'bg-white' ? 'bg-blue-50 group-hover:bg-[#0a45e8] transition-colors' : 'bg-white/20 backdrop-blur-md'
                }`}>
              ...
              <h3 className={`text-[22px] font-bold mb-4 leading-snug ${feature.bg === 'bg-white' ? 'text-gray-900 group-hover:text-[#0a45e8] transition-colors' : 'text-white'
                }`}>
                {feature.title}
              </h3>
              <p className={`text-[15px] leading-relaxed flex-grow ${feature.bg === 'bg-white' ? 'text-gray-500' : 'text-blue-100 opacity-90'
                }`}>
                {feature.description}
              </p>
            </div>
